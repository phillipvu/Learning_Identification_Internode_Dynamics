...
% one resample() method to call
    ...
    methods
        function self = hsmm(T,obs_distns,dur_distns,alpha,gamma,rho,trunc,trans_distn,init_state_distn)
            ...
            if ~exist('trans_distn','var')
                self.trans_distn = transitions(state_dim,alpha,gamma);
            else
                self.trans_distn = trans_distn;
            end

            if ~exist('init_state_distn','var')
                self.init_state_distn = initial_state(state_dim,rho);
            else
