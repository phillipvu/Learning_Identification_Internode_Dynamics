...
M = 200; % downsampling factor
infile = 'trialgmsk3'; % name of USRP trace file to be read in
data = read_complex_binary(infile)';    % reads in trace binary data
data = data(250e3:end);                 % truncate to remove USRP turn-on
                                        % transient
...
dur_hypparams1 = {250,2};
dur_hypparams2 = {750,1.5};
dur_hypparams3 = {750,1.5};
dur_hypparams4 = {750,1.5}; 
obs_hypparams1 = {zeros(obs_dim,1),eye(obs_dim),obs_dim+3,10}; 
obs_hypparams2 = {zeros(obs_dim,1),eye(obs_dim),obs_dim+20,5}; 
obs_hypparams3 = {zeros(obs_dim,1),eye(obs_dim),obs_dim+25,5}; 
obs_hypparams4 = {0.005*ones(obs_dim,1),eye(obs_dim),obs_dim+30,5}; 
...
dur_limit = 5;
[transition_matrix,full_transition_matrix,new_state_idx,~,new_stateseq,statemap1] = remove_states(posteriormodel, dur_limit);
